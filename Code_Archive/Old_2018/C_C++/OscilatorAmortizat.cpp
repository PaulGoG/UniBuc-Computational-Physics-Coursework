...
ofstream f ("OscAmort1.txt");
ofstream g ("OscAmort2.txt");
ifstream m ("OscAmort1.txt");
ifstream n ("OscAmort2.txt");
ofstream d ("DistantaAmort.txt");
ifstream p ("DistantaAmort.txt");
ofstream timp ("TimpiUCAmort.txt");
...
double functiey(double x, double y)
{
    return -omega*omega*x-2*delta*y;
}
void RK41(double x, double y,double h, int N)
{
    ...
    for(int i=1;i<=N;i++)
    {
        ...
        k2[2]=functiey(x+h*k1[1]/2,y+h*k1[2]/2);

        k3[1]=functiex(y+h*k2[2]/2);
        k3[2]=functiey(x+h*k2[1]/2,y+h*k2[2]/2);

        k4[1]=functiex(y+h*k3[2]);
        k4[2]=functiey(x+h*k3[1],y+h*k3[2]);
