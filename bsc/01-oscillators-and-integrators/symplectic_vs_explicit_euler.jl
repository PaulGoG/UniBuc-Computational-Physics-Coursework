...
function main()
    ...
    for (k, (step, name, colour)) in enumerate(SCHEMES)
        ax = Axis(fig[2, k], aspect = DataAspect(),
            xlabel = L"Position $x$", ylabel = k == 1 ? L"Velocity $v$" : "",
            title = name, titlesize = 17)
        for (x₀, v₀) in starts
    ...
    for (step, name, colour) in SCHEMES
        ...
        scatterlines!(ax_n, Ns, errs, color = colour, linewidth = 1.3, markersize = 7)
    end

    Legend(fig[1, 1:3], handles, [s[2] for s in SCHEMES],
